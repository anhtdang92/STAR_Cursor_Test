...
class AutoencoderKLTemporalDecoder(nn.Module):
    def __init__(
        ...
        self.loss = instantiate_from_config(ddconfig["loss"])
        ...
        if ema_decay is not None:
            ...
            self.ema = LitEma(self, decay=ema_decay)
        ...
        if not only_inputs:
            ...
            if xrec.shape[1] > 3:
                ...
                for log in self.loggers:
                    if isinstance(log, ImageLogger):
                        log.log_img(
                            self, torch.cat([inputs, x], dim=0), self.global_step, split="test"
                        )
            else:
                x = xrec
            if use_ema:
                self.ema.restore()
            return {"inputs": inputs, "reconstructions": x}
        if use_ema:
            self.ema.restore()
...
class Normalize(nn.Module):
    def __init__(self, dim, eps=1e-5):
        super().__init__()
        self.eps = eps
        self.g = nn.Parameter(torch.ones(1, dim, 1, 1) * 0.02)
